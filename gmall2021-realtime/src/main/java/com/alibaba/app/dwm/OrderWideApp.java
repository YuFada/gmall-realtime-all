...
public class OrderWideApp {
    public static void main(String[] args) throws Exception {
        ...
        */
        ...
        SingleOutputStreamOperator<OrderDetail> orderDetailDStream = orderDetailJsonDStream.map(new RichMapFunction<String, OrderDetail>() {
            SimpleDateFormat simpleDateFormat = null;
...
//        orderDetailDStream.print("orderDetail::::");
        ...
        //TODO 2.设定事件时间水位
        SingleOutputStreamOperator<OrderInfo> orderInfoWithEventTimeDstream = orderInfoDStream.
        ...
        //TODO 3.设定关联的key
        KeyedStream<OrderInfo, Long> orderInfoKeyedDstream = orderInfoWithEventTimeDstream
                .keyBy(orderInfo -> orderInfo.getId());
        KeyedStream<OrderDetail, Long> orderDetailKeyedStream = orderDetailWithEventTimeDstream
                .keyBy(orderDetail -> orderDetail.getOrder_id());
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithUserDstream = AsyncDataStream.unorderedWait(
                orderWideDstream, new DimAsyncFunction<OrderWide>("DIM_USER_INFO") {
                    ...
                    public void join(OrderWide orderWide, JSONObject jsonObject) throws Exception {
                        ...
                        Long ageLong = betweenMs / 1000L / 60L / 24L / 365L;
        ...
        //orderWideWithTmDstream.map(orderWide -> JSONObject.toJSONString(orderWide)).
        //        addSink(MyKafkaUtil.getKafkaSink(orderWideSinkTopic));
        SingleOutputStreamOperator<String> ds = orderWideWithTmDstream.map(new MapFunction<OrderWide, String>() {
            @Override
            public String map(OrderWide orderWide) throws Exception {
                return JSONObject.toJSONString(orderWide);
            }
        });
        ds.addSink(MyKafkaUtil.getKafkaSink(orderWideSinkTopic));
