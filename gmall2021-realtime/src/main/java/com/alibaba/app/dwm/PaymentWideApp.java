...
public class PaymentWideApp {
    public static void main(String[] args) throws Exception {
        ...
        */
        ...
        //对读取的支付数据进行转换
        //SingleOutputStreamOperator<PaymentInfo> paymentInfoDStream  = paymentInfojsonDstream.
        //        map(jsonString -> JSON.parseObject(jsonString, PaymentInfo.class));

        SingleOutputStreamOperator<PaymentInfo> paymentInfoDStream = paymentInfojsonDstream.map(new MapFunction<String, PaymentInfo>() {
            @Override
            public PaymentInfo map(String value) throws Exception {
                return new PaymentInfo(JSON.parseObject(value));
            }
        });
